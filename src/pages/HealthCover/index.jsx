...
import {fetchHealthCover, fetchHealthCoverPage} from "../../api/redux/HealthCover";
...
const HealthCover = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.HEALTH_COVER
        dispath(fetchHealthCover([api_url]))

        let api_url2 = apiEndPoints?.GET_HEALTH_COVER
        dispath(fetchHealthCoverPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.healthCover)
    ...
    return (
        <HelmetProvider>
            ...
            <StyledInternationStudy>
                <InnerBanner first={'Student Essential'} second={innerBanner?.subtitle} title={innerBanner?.subtitle}
                        ...
                        <Col md={8} className={'study-abroad-content'}>
...

                            {
                                studyCard && studyCard?.map((item, index) => {
                                    return (
                                        <Content key={index} paraOne={item?.post_content} linkText={item?.post_title} url={`/health-cover/${item?.post_name}`} />
                                    )
                                })
                            }
                            ...
                            {
                                table && <CustomTable mt data={table}/>
                            }

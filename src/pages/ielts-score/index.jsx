...
import {fetchIeltsScore, fetchIeltsScorePage} from "../../api/redux/IeltsScore";
...
const IeltsScore = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.IELTS_SCORE
        dispath(fetchIeltsScore([api_url]))

        let api_url2 = apiEndPoints?.GET_IELTS_SCORE
        dispath(fetchIeltsScorePage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.ieltsScore)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="IELTS Score"
                      content="In today's globally connected world, studying abroad has become a cherished dream for many Bangladeshi students."/>
            </Helmet>
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/ielts-score/${item?.post_name}`} key={index} img={item?.post_feature_image}

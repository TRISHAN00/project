...
import {fetchStudyInAustralia, fetchStudyInAustraliaPage} from "../../api/redux/StudyInAustralia";
...
const StudyInAustralia = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.STUDY_IN_AUSTRALIA
        dispath(fetchStudyInAustralia([api_url]))

        let api_url2 = apiEndPoints?.GET_STUDY_IN_AUSTRALIA
        dispath(fetchStudyInAustraliaPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.studyInAustralia)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="STUDY IN AUSTRALIA"
                      content="Studying in Australia has become an increasingly popular choice for Bangladeshi students seeking quality education and global exposure. "/>
            </Helmet>
            <StyledInternationStudy>
                <InnerBanner first={'Home'} firstUrl={'/'} second={'countries'} title={innerBanner?.subtitle}
                ...
                <Container>
                    <Row>
                        ...
                        <Col md={8} className={'study-abroad-content'}>
                            ...
                            {
                                pageContent && <Content paraOne={pageContent} />
                            }
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/study-in-australia/${item?.post_name}`} key={index} img={item?.post_feature_image}

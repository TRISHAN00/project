...
import {fetchStudyInAustraliaDetails} from "../../api/redux/StudyInAustralia";

const StudyInAustraliaDetails = () => {

    const dispatch = useDispatch()
    const getData = useSelector(store => store.studyInAustralia)
    let {slug} = useParams();

    // api call
    useEffect(() => {
        let api_url = apiEndPoints.STUDY_IN_AUSTRALIA_DETAILS
        dispatch(fetchStudyInAustraliaDetails([api_url + `/${slug}`]))
    }, [])
...
"/>
            ...
            <StyledInternationStudy>
                ...
                {
                    cta && <Feature leftUrl={cta?.left_url} rightUrl={cta?.right_url} subtitle={cta?.subtitle} leftText={cta?.left_text} rightTopSmall={cta?.right_top} rightBottomTitle={cta?.right_bottom} />
                }
...
export default StudyInAustraliaDetails;

...
import {fetchStudyInUk, fetchStudyInUkPage} from "../../api/redux/StudyInUk";
...
const StudyInUk = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.STUDY_IN_UK
        dispath(fetchStudyInUk([api_url]))

        let api_url2 = apiEndPoints?.GET_STUDY_IN_UK
        dispath(fetchStudyInUkPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.studyInUk)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Studying in the UK"
                      content="Studying in the UK Studying in the UK is a dream for many Bangladeshi students due to its prestigious universities, high-quality education, and diverse cultural experiences. "/>
            </Helmet>
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/study-in-uk/${item?.post_name}`} key={index} img={item?.post_feature_image}

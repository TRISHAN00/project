...
import {fetchStudyInUsa, fetchStudyIsUsaPage} from "../../api/redux/StudyInUsa";
...
const StudyInUsa = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.STUDY_IN_USA
        dispath(fetchStudyInUsa([api_url]))

        let api_url2 = apiEndPoints?.GET_STUDY_IN_USA
        dispath(fetchStudyIsUsaPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.studyInUsa)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Study In USA"
                      content="studying in the United States offers numerous benefits for Bangladeshi students."/>
            </Helmet>
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/study-in-usa/${item?.post_name}`} key={index} img={item?.post_feature_image}

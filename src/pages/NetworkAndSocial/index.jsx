...
import {fetchNetworkAndSocial} from "../../api/redux/NetworkAndSocial";

const NetworkAndSocial = () => {

    const dispath = useDispatch();

    // api call
    useEffect(() => {
        let api_url = apiEndPoints.NETWORK_AND_SOCIAL
        dispath(fetchNetworkAndSocial([api_url]))
    }, [dispath])

    let getPost = useSelector(state => state.networkAndSocial)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Networking and socializing"
                      content="Networking and socializing are crucial for international students who are studying in a new country. Making connections with others not only helps to build friendships"/>
            </Helmet>
            <StyledInternationStudy>
                <InnerBanner first={'Home'} firstUrl={'/'} second={'Living Abroad'} title={innerBanner?.subtitle}
...
export default NetworkAndSocial;

...
import {fetchPageWhyStudyAbroad, fetchWhyStudyAbroad} from "../../api/redux/WhyStudyAbroad";
...
const WhyStudyAbroad = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.WHY_STUDY_ABROAD
        dispath(fetchWhyStudyAbroad([api_url]))

        let api_url2 = apiEndPoints?.GET_WHY_STUDY_ABROAD
        dispath(fetchPageWhyStudyAbroad([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.whyStudyAbroad)

    const pageContent = getPost?.posts?.page_data?.[0]?.post_content;
    ...
    const studyCard = getPost?.page_data;
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Top reasons to pursue higher education overseas"
                      content="Learn how to study abroad with these easy-to-follow steps for international education.
"/>
            </Helmet>
            <StyledInternationStudy>
                <InnerBanner first={'Home'} firstUrl={'/'} second={'Study Abroad'} title={innerBanner?.subtitle}
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/why-study-abroad/${item?.post_name}`} key={index}
                                                  img={item?.post_feature_image}
                ...
                <Feature subtitle={cta?.subtitle} leftText={cta?.left_text} rightTopSmall={cta?.right_top}
                         rightBottomTitle={cta?.right_bottom} leftUrl={cta?.left_url} rightUrl={cta?.right_url}/>

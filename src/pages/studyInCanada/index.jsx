...
import {fetchStudyInCanada, fetchStudyInCanadaPage} from "../../api/redux/StudyInCanada";
...
const StudyInCanada = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.STUDY_IN_CANADA
        dispath(fetchStudyInCanada([api_url]))

        let api_url2 = apiEndPoints?.GET_STUDY_IN_CANADA
        dispath(fetchStudyInCanadaPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.StudyInCanada)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Studying in Canada"
                      content="Canada is one of the top choices for international students seeking quality education. "/>
            </Helmet>
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/study-in-canada/${item?.post_name}`} key={index} img={item?.post_feature_image}

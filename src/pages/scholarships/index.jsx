...
import {fetchScholarships, fetchScholarshipsPage} from "../../api/redux/Scholarships";
...
const Scholarships = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.SCHOLARSHIPS
        dispath(fetchScholarships([api_url]))

        let api_url2 = apiEndPoints?.GET_SCHOLARSHIPS_DETAILS
        dispath(fetchScholarshipsPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.scholarships)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Scholarship Opportunities for Bangladeshi Students"
                      content="This prestigious scholarship program offers opportunities for Bangladeshi students to pursue masters or doctoral degrees in the USA."/>
            </Helmet>
                            ...
                            <Row>
                                {
                                    studyCard?.map((item, index) => {
                                        return (
                                            <Card url={`/scholarships/${item?.post_name}`} key={index} img={item?.post_feature_image}

...
import {fetchUniversityRanking, fetchUniversityRankingPage} from "../../api/redux/UniversityRanking";
...
const UniversityRanking = () => {
    ...
    // api call
    useEffect(() => {
        let api_url = apiEndPoints?.UNIVERSITY_RANKING
        dispath(fetchUniversityRanking([api_url]))

        let api_url2 = apiEndPoints?.GET_UNIVERSITY_RANKING
        dispath(fetchUniversityRankingPage([api_url2]))
    }, [dispath])

    let getPost = useSelector(state => state?.universityRanking)
    ...
    return (
        <HelmetProvider>
            <Helmet>
                ...
                <meta name="Choosing a University Abroad Based on Rankings"
                      content="Studying abroad is an exciting opportunity for Bangladeshi students to broaden their horizons, gain a global perspective, and acquire quality education."/>
            </Helmet>
            <StyledInternationStudy>
                <InnerBanner first={'Home'} firstUrl={'/'} second={'course'} title={innerBanner?.subtitle}
                ...
                <Container>
                    <Row>
                        ...
                        <Col md={8} className={'study-abroad-content'}>
                            ...
                            {
                                studyCard && studyCard?.map((item, index) => {
                                    return (
                                        <Content key={index} paraOne={item?.post_content} linkText={item?.post_title} url={`/university-ranking/${item?.post_name}`} />
                                    )
                                })
                            }
